...
public interface StandardMapper extends BaseMapper<Standard> {
    @Update("update good_standard set store = store - #{num} where good_id = #{good_id} and value = #{standard} and store >= #{num}")
    int deductStore(@Param("good_id") long good_id, @Param("standard") String standard, @Param("num") int num);
